...
import { addPolicy, updatePolicy } from "@/data/mock";
...
export function PolicyFormDialog({ policy, open, onOpenChange, onSave }: PolicyFormDialogProps) {
  ...
  function handleSave() {
    ...
    if (isEdit && policy) {
      const updated: Policy = { ...policy, name: form.name.trim(), rules };
      updatePolicy(updated);
      onSave(updated);
      toast.success("Política atualizada", { description: `${updated.name} foi atualizada.` });
    } else {
      const newPolicy: Policy = { id: `policy_${Date.now()}`, name: form.name.trim(), rules };
      addPolicy(newPolicy);
      onSave(newPolicy);
      toast.success("Política criada", { description: `${newPolicy.name} foi adicionada à governança.` });
    }
    onOpenChange(false);
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? "Atualize as regras de governança desta política." : "Defina uma nova regra de governança entre módulos."}
          </DialogDescription>
        </DialogHeader>
